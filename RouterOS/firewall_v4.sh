...
add address=10.0.0/24 comment="defconf: local LAN IPv4 subnet" list=local_subnet_ipv4
add address=10.0.0.2 comment="lanconf: local DNSv4 server" list=local_dns_ipv4
add address=10.0.0.4 comment="lanconf: local DNSv4 server" list=local_dns_ipv4
...
add action=fasttrack-connection chain=forward comment="defconf: fasttrack" connection-state=established,related in-interface=pppoe-out1
...
add action=drop chain=forward comment="defconf: drop all from WAN not DSTNATed" connection-nat-state=!dstnat in-interface-list=WAN log=yes log-prefix="[wan-not-dnat]"
add action=drop chain=forward comment="onuconf: drop all from ONU not DSTNATed" connection-nat-state=!dstnat in-interface-list=ONU log=yes log-prefix="[onu-not-dnat]"
...
add action=accept chain=prerouting comment="defconf: accept inter-interface router traffic" src-address-type=local
